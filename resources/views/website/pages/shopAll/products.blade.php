        ...
        <section class="pt-0 flat-spacing">
            <div class="container">
                @include('website.pages.shopAll.productsControl')
                        ...
                        <div class="grid card-product" data-availability="Out of stock" data-brand="adidas">
                            <div class="card-product-wrapper">
                                <a href="{{ route('product.show', $product->slug) }}" class="product-img">
                                    <img class="lazyload img-product" data-src="{{ $product->getFirstMediaUrl('product_thumbnail') }}" src="{{ $product->getFirstMediaUrl('product_thumbnail') }}" alt="image-product">
                                    <img class="lazyload img-hover" data-src="{{ $product->getFirstMediaUrl('product_thumbnail') }}" src="{{ $product->getFirstMediaUrl('product_thumbnail') }}" alt="image-product">
                                </a>
                                <div class="list-product-btn">
                                    <a href="javascript:void(0);" class="box-icon wishlist btn-icon-action btn-add-to-wishlist" data-product-id="{{ $product->id }}" data-in-wishlist="false">
                                ...
                                <div class="list-btn-main">
                                    @if($product->isSimple())
                                        <a href="javascript:void(0);" class="btn-main-product" data-add-to-cart data-product-id="{{ $product->id }}" data-product-type="simple">Add To cart</a>
                                    @elseif($product->isVariant())
                                        <a href="javascript:void(0);" class="btn-main-product" data-bs-toggle="modal" data-bs-target="#quickView{{ $product->id }}">Select Options</a>
                                    @elseif($product->isBundle())
                                        ...
                                        @if($hasBundleVariants)
                                            <a href="javascript:void(0);" class="btn-main-product" data-bs-toggle="modal" data-bs-target="#bundleSelectionModal{{ $product->id }}">Select Bundle Items</a>
                                        @else
                                            <a href="javascript:void(0);" class="btn-main-product" data-add-to-cart data-product-id="{{ $product->id }}" data-product-type="bundle">Add To cart</a>
                                        @endif
                                    @endif
                                </div>
                            </div>
        ...
        @include('website.pages.shopAll.filter')
